...
import { Flex, Text, Card, Table, Badge, Box, Tooltip, Spinner, Button, Dialog, Code } from '@radix-ui/themes'
...
export const SpotlightSharedSessionPage: React.FC = () => {
  ...
  const { data: sessionDetail, isLoading: loadingDetail, error } = useQuery<SessionDetail>(
    ['spotlight-shared-session-detail', shareToken],
    async () => {
      const response = await axios.get(
        `${BACKEND_URL}/spotlight/analytics/sessions/shared/${shareToken}`
      )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--gray-1)' }}>
      <Flex direction="column" gap="4" style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
        {/* Header */}
        <Flex direction="column" gap="2">
          <Flex align="center" gap="2">
            <Text size="6" weight="bold">Shared Session</Text>
            <Badge color="blue" variant="soft">View Only</Badge>
          </Flex>
        ...
        {loadingDetail ? (
          ...
          <Card>
            <Flex direction="column" align="center" justify="center" style={{ padding: '48px' }}>
              <Text size="3" color="red">This shared session is not available</Text>
              <Text size="2" color="gray">It may have been unshared or deleted</Text>
            </Flex>
          </Card>
        ) : sessionDetail ? (
